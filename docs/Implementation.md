...
Scheduling of a device is specified by configuration:
```
...
#### Analogue ports
A device can also register an analogue port. Such ports are usually associated with Analogue-to-Digital converters or similar. It is also possible to connect such ports and their values
